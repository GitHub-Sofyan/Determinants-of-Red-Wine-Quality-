...
## Information on the variables can be found here.

## Udacity DAND Project requirement
## Complete Univariate - Bivariate - Multivariate Plot

# Preparation for Battle - Loading of all (level = standard) relevant dependancies and libaries (that will be required for the EDA)
```{r}
library(ggplot2)
library(dplyr)
library(e1071)  
...
# Loading the Battlefield and Reconnaisance of Data (Part I)
```{r}
...
```{r}
transform(wine, quality = as.numeric(quality)) # Change quality from int to dbl. It is the only column that is not a dbl. This was added post-hoc due to several ploting errors below. Therefore added this her to minimise errors later. 
```
...
# Plotting the Battlefield Layout (Distribution of wine quality)
```{r}
...
# Reconnaisance of Data (Part II) - Correlation
...
modelEstimate.example = predict(m3, newdata = trialwine, interval="prediction", level = .95)
...
```{r}
trialwine8 = data.frame(alcohol= 10, sulphates= 0.47, volatile.acidity = 0.5, citric.acid = 0.00, fixed.acidity = 7.3, density = 0.99460)
modelEstimate.example = predict(m6, newdata = trialwine8, interval="prediction", level = .95)
predicted.quality.trial <- round(as.data.frame(modelEstimate.example)$fit,0)
answer <- as.numeric(predicted.quality.trial)
print(answer)
```
...
```{r}
trialwine8 = data.frame(alcohol= 10, sulphates= 0.47, volatile.acidity = 0.5, fixed.acidity = 7.3, density = 0.99460, pH = 3.39)
modelEstimate.example = predict(m8, newdata = trialwine8, interval="prediction", level = .95)
predicted.quality.trial <- round(as.data.frame(modelEstimate.example)$fit,0)
answer <- as.numeric(predicted.quality.trial)
print(answer)
```
...
### OPTIONAL: Did you create any models with your dataset? Discuss thestrengths and limitations of your model.
...
biplot(pcx, xlab=rep('.', var_length)) #plotting using biplot
```
## This is the plot that started it all. It allowed the investigators to look at the big picture and use the results of the plot to guide their journey through the various variables. As mentioned earlier, the PCA lead to the investigation of alcohol as the first variable. 
...
ggplot(data=wine, aes(y=alcohol, x=quality)) + 
  ...
  ggtitle("Influence of Alcohol percentage on the Quality of Wine")
```

### This plot shows the clear influence of alcohol on wine quality, howecver as observed in later plots and modelling, it is not the only determinant. At the same time, it shows the lack of data on the tail ends of the dataset (3, 4, 7 & 8). Therefore this plot place is justified here as it shows that perhaps a better prediction model or information could have been obtained with a better data set. Nevertheless we can conclude alcohol's role on wine quality. 


# Plot 3 - Influence of Alcohol on Quality of Wine

```{r}
p1 <- ggplot(aes(x = volatile.acidity, y = citric.acid),
       data = wine) +
  geom_point(aes(color = quality)) + theme_bw() +
  geom_violin(aes(color = quality)) +
  xlab('Volatile acidity(g/dm^3)') + 
  ylab('Citric acid(g/dm^3)') +
  theme(legend.position="bottom")

p2 <- ggplot(aes(x = volatile.acidity, y = sulphates),
       data = wine) +
  geom_point(aes(color = quality)) + theme_bw() +
  geom_violin(aes(color = quality)) +
  xlab('Volatile acidity(g/dm^3)') + 
...
p3 <- ggplot(aes(x = volatile.acidity, y = round(alcohol)),
       data = wine) +
  geom_jitter(aes(color = quality), 
              position = position_jitter(h = 0.3, w = 0)) +
  geom_violin(aes(color = quality)) + theme_bw() +
  xlab('Volatile acidity(g/dm^3)') + 
  ylab('Alcohol(% by volume)') +
  theme(legend.position="bottom")

grid.arrange(p1, p2, p3, ncol = 3,bottom = paste('Pairwise comparison of the 6 influential variables,','with density maps (grouped by quality)'))
```
```{r}
p4 <- ggplot(aes(x = citric.acid, y = sulphates),
       data = wine) +
  geom_point(aes(color = quality)) + theme_bw() +
  geom_violin(aes(color = quality)) +
...
p5 <- ggplot(aes(x = citric.acid, y = round(alcohol)),
       data = wine) +
  geom_jitter(aes(color = quality), 
              position = position_jitter(h = 0.3, w = 0)) +
  geom_violin(aes(color = quality)) + theme_bw() +
...
grid.arrange(p4, p5, p6, ncol = 3, bottom = paste('Pairwise comparison of the 6 influential variables,','with density maps (grouped by quality)'))
```
### This final plot is selected as it attempts to show the distribution of the variables against quality. While there is an overal, you are able to see the difference in shapes for each different quality, which suggest that there a difference in these variables could mean a diference in the wine quality. Only 6 of these variables have been selected based on the results of earlier EDA and plotting. 
...
### 1) Data Set: First, outliers or rather the lack of removing/treatment it. Usually ohe of the very first steps would be to investigate outliers, as the outliers could be possibly due to several other reasons such as measurement error etc. 
